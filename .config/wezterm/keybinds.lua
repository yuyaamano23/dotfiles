  ...
  return {
    ...
    action = wezterm.action_callback(function(win, pane)
      if pane:is_alt_screen_active() then
        -- Neovim 等の Alt Screen が有効な場合はキーをそのまま送る
        -- あなたの環境の Leader (Ctrl+j) と移動キーを送ります
...
return {
  keys = {
    -- ===== Pane split (LEADER: Ctrl+w) =====
    leader("\\", act.SplitHorizontal { domain = "CurrentPaneDomain" }), -- 左右分割
    leader("-", act.SplitVertical   { domain = "CurrentPaneDomain" }), -- 上下分割
    ...
    -- ===== Tab move (Ctrl + [ / ]) =====
    {
      ...
      action = wezterm.action_callback(function(win, pane)
        if pane:is_alt_screen_active() then
          -- Neovim起動中は、キーをそのままNeovimへ送る
          win:perform_action(act.SendKey { key = '[', mods = 'CTRL' }, pane)
        else
          -- 通常時は WezTerm のタブを左へ移動
    ...
    -- Ctrl + w で git worktree マネージャーを起動する設定例
    {
      key = 'w',
      mods = 'CTRL',
      action = wezterm.action.SendString 'gwt\n',
    },

    -- 【最優先】Google 日本語入力の邪魔をしないための保険
    {
      key = 'k',
      mods = 'CTRL',
